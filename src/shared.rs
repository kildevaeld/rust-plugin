...
pub trait SyncPluggable {
    ...
    where P::Value: Clone + Any + Send + Sync, Self: SyncExtensible {
    ...
    where P::Value: Any + Send + Sync, Self: SyncExtensible {
...
mod test {
    ...
    use typemap::{ShareMap, Key, TypeMap};
    use super::{SyncExtensible, Plugin, SyncPluggable};
    ...
    impl Extended {
        fn new() -> Extended {
            Extended { map: TypeMap::custom() }
        }
    }

    impl SyncExtensible for Extended {
        fn extensions(&self) -> &ShareMap { &self.map }
        fn extensions_mut(&mut self) -> &mut ShareMap { &mut self.map }
    }

    impl SyncPluggable for Extended {}
